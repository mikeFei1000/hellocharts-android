...
public class FloatUtils {
     ...
     * @param outValues The destination {@link AxisAutoValues} object to populate.
     */
    public static void computeAutoGeneratedAxisValuesNoCroop(float start, float stop, int steps, AxisAutoValues outValues) {
        double range = stop - start;
        if (steps == 0 || range <= 0) {
            outValues.values = new float[]{};
            outValues.valuesNumber = 0;
        ...
        int valueIndex;
        int valuesNum = 0;
        for (intervalValue = first; intervalValue <= last + interval; intervalValue += interval) {
            ++valuesNum;
        }
